...
byte currentNote[] = {0, 0, 0, 0, 0}; //current MIDI note
...
void handleNoteOn(byte channel, byte pitch, byte velocity) //MIDI Note ON Command
{
  ...
  currentNote[channel] = pitch; //save current pitch for later
...
{
  if (currentNote[channel] == pitch) { //Checks if pitch is current note playing
    currentNote[channel] = 0; //reset currentNote
...
void singleStep(byte motorNum, byte stepPin)
{
  ...
  { //step when correct time has passed and the motor is at a nonzero speed
    prevStepMicros[motorNum] = micros();
